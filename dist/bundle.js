...
function l(t, a) {
  ...
  if (l < 1) r = 0;
    ...
    1 === o.getMonth() && o.getDate() > 27 && o.setDate(30),
      o.setMonth(o.getMonth() - s * l);
    var m = i(o, u) === -s;
    d(n(t)) && 1 === l && 1 === i(t, u) && (m = !1), (r = s * (l - Number(m)));
  }
  return 0 === r ? 0 : r;
}
function m(t, a, r) {
...
function T(t, a, u) {
  ...
  var h = r(),
    c =
      null !==
        (s =
          null !== (d = null == u ? void 0 : u.locale) && void 0 !== d
            ? d
            : h.locale) && void 0 !== s
        ? s
        : w;
  ...
  if (k < W) {
    var j = Math.round(k / D);
    return c.formatDistance("xDays", j, y);
  }
  if (k < P)
    return (b = Math.round(k / W)), c.formatDistance("aboutXMonths", b, y);
  if ((b = l(v, g)) < 12) {
    var C = Math.round(k / W);
    return c.formatDistance("xMonths", C, y);
  }
  var X = b % 12,
    A = Math.floor(b / 12);
  return X < 3
    ? c.formatDistance("aboutXYears", A, y)
    : X < 9
    ? c.formatDistance("overXYears", A, y)
    : c.formatDistance("almostXYears", A + 1, y);
}
const k = document.getElementById("comic-img"),
  x = document.getElementById("title"),
  j = document.getElementById("date");
!(async function () {
  const t = await (function (t) {
      const e = new URLSearchParams();
      return (
        e.append("email", t),
        fetch("https://fwd.innopolis.university/api/hw2?" + e.toString()).then(
          function (t) {
            return t.json();
          },
        )
      );
    })("d.koncheva@innopolis.university"),
    e = await (function (t) {
      return fetch(
        "https://fwd.innopolis.university/api/comic?id=" + t.toString(),
      ).then(function (t) {
        return t.json();
      });
    })(t);
  null != x &&
    null != j &&
    null != k &&
    ((x.textContent = e.safe_title),
    k.setAttribute("alt", e.alt),
    k.setAttribute("src", e.img),
    new Date(e.year, e.month, e.day),
    (j.textContent = T(new Date(e.year, e.month - 1, e.day), new Date(), {
      addSuffix: !0,
    })));
})();
